...
interface FilterSidebarProps {
  filters: any; // Using any for brevity here, normally we'd type this strictly from the hook
  onToggleArray: (key: string, item: string) => void;
  onUpdateValue: (key: string, value: any) => void;
  className?: string;
}

export function FilterSidebar({ filters, onToggleArray, onUpdateValue, className = '' }: FilterSidebarProps) {
          ...
          <label key={option} className="flex items-center gap-3 cursor-pointer group">
            <div className="relative flex items-center justify-center">
              <input 
                ...
                onChange={() => onToggleArray(filterKey, option)}
              />
  ...
  return (
    <aside className={`w-full bg-white rounded-xl shadow-sm border border-gray-100 p-6 ${className}`}>
      ...
      <CheckboxGroup title="State" filterKey="state" options={INDIAN_STATES} selected={filters.states} />
      <hr className="my-6 border-gray-100" />
      
      <CheckboxGroup title="Course" filterKey="course" options={COURSE_TYPES} selected={filters.courses} />
      <hr className="my-6 border-gray-100" />
      
      <CheckboxGroup title="Exam Accepted" filterKey="exam" options={EXAM_TYPES} selected={filters.exams} />
      <hr className="my-6 border-gray-100" />

      <CheckboxGroup title="College Type" filterKey="type" options={COLLEGE_TYPES} selected={filters.types} />
      <hr className="my-6 border-gray-100" />
      ...
      <div className="mb-2">
        ...
        <div className="px-2 mt-4">
          <input 
            ...
            onChange={(e) => onUpdateValue('maxFees', e.target.value)}
            className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-600"
          />
          <div className="flex justify-between text-xs text-gray-500 mt-2 font-medium">
            <span>₹50K</span>
            <span className="text-blue-700 font-bold">{formatCurrency(filters.maxFees)}</span>
            <span>₹30L+</span>
          </div>
        </div>
      </div>
